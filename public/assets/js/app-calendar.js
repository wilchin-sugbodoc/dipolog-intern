$(function () {
  ...
  var generateTime = function (element) {
    ...
    for (var i = 0; i < hours.length; i++) {
      ...
      while (n < 60 - min) {
        times.push(
          hours[i] + ":" + ((n += min) < 10 ? "O" + n : n) + periods[0]
        );
      }
      n = 0;
    }
  ...
  $(".main-nav-calendar-event a").on("click", function (e) {
    ...
    if ($(this).hasClass("exclude")) {
      ...
      $(this).is(":first-child")
        ? azCalendar.addEventSource(sptCalendarEvents)
        : "";
      $(this).is(":nth-child(2)")
        ? azCalendar.addEventSource(sptBirthdayEvents)
        : "";
      $(this).is(":nth-child(3)")
        ? azCalendar.addEventSource(sptHolidayEvents)
        : "";
      $(this).is(":nth-child(4)")
        ? azCalendar.addEventSource(sptOtherEvents)
        : "";
    } else {
